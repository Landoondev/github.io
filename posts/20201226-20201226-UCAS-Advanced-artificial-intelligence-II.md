...
> 传教士和野人问题通常描述如下：三个传教士和三个野人在河的一边，还有一条能载一个人或者两个人的船。找到一个办法让所有的人能渡到河的另一岸，要求在任何地方野人数都不能多于传教士的人数。（野人数量如果大于传教士，那么野人合伙起来就是欺负传教士。传教士的数量是可以为 0 的。）
>
...
- $S$：非终止状态集 $S=\{1, 2, 3, 4\}$
- $A$：对于 $S$ 中每个状态有 4 种可能的动作 $A = \{ up, down, left, right \}$
- $P$：当前状态 $s$ 选择任意动作 $a$ 后只有一个确定的下一状态和收益。因此 $p(s', r \mid s, a) = 1$。
- $R$：在达到终止状态之前，所有的动作收益 $R=-1$。
- $\gamma$：$\gamma=1$ 表示无折扣。

![](./20201226/22.jpeg)

假如环境已知 $p(s', r \mid s, a)$ 已知，状态估值函数的求解方式有：
...
>
> > 分析：假设四个格子的状态估值分别记为 $V_1, V_2, V_3, V_4$。
> >
> > 根据贝尔曼方程有：
> >
> > $$V_1 = \frac{1}{4}((-1 + 0) + (-1 + V_1) + (-1+V_1) + (-1+V_2))$$
> >
> > $$\Rightarrow V_1 = \frac{1}{4}(0 + V_1 + V_1 + V_2) + \frac{1}{4}(-1-1-1-1) $$
> >
> > OK.
>
> 解之得：$V_1=-8, V_2=-12, V_3 = -8, V_4 = -16.$
>
> （2）策略提升（第一问对应的贪心策略）：
>
> ![](./20201226/23.png)
...
> $$V_1 =\frac{1}{1} (-1+0)$$
>
> $$V_2 = \frac{1}{2} ((-1+V_1) +(-1 + V_3))$$
>
> $$V_3 = \frac{1}{1}(-1+0)$$
>
> $$V_4 = \frac{1}{1}(-1+V_2)$$
>
> > $\pi(a \mid s)$ 发生了改变，经过贪心策略后，$a$（Action） 变少了（$V_2$ 只可以向左和向右）。
> >
> > - $$\pi(up \mid s)=0, \pi(down \mid s)=0, \pi(left \mid s)=0.5, \pi(right \mid s)=0.5$$
> > - 这就是 $V_2​$ 中 1/2 的来源了。
>
> 解之得：$V_1=-1, V_2 = -2, V_3=-1, V_4=-3.$
>
> ![](./20201226/24.png)
